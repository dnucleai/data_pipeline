        ...
        if CSVParser.is_csv(dataset_source):
            try:
                with timeout(30, exception=RuntimeError):
                    print("Beginning download from " + dataset_source)
                    if online:
                    ...
                    es_documents = [Document(dataset_name, raw_document).get_es_document() for raw_document in
                                    raw_documents]
                    self.es.bulk_upload(es_documents)
            except RuntimeError:
                print("Downloading took too long, moving on.")
                pass
